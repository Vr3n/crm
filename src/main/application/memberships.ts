...
export function sellMembership(input: SellMembershipInput): SellMembershipResult {
  ...
  if (existing?.response) {
    try { return JSON.parse(existing.response) as SellMembershipResult } catch {}
  }
  if (input.discountType === 'NONE' && input.discountValueMinor !== null) throw new ValidationError('discountValue must be null when discountType is NONE')
  if (input.discountType !== 'NONE' && input.discountValueMinor === null) throw new ValidationError('discountValue is required when discountType is not NONE')
  const start = new Date(`${input.startDate}T00:00:00`)
  const end = new Date(`${input.endDate}T00:00:00`)
  const joining = new Date(`${input.joiningDate}T00:00:00`)
  if (Number.isNaN(start.getTime()) || Number.isNaN(end.getTime()) || Number.isNaN(joining.getTime())) throw new ValidationError('Invalid joining, start, or end date')
  ...
  if (input.endDate < minEnd) throw new ValidationError(`End date violates ${plan.duration} — should be at least ${minEnd} (${expectedDays} days)`)
  ...
  if (input.offerId !== null) {
    ...
    if (offer.validTo !== null && offer.validTo < today) throw new ValidationError('Offer has expired')
    if (offer.applicablePlanIds.length > 0 && !offer.applicablePlanIds.includes(plan.id)) throw new ValidationError(`Offer "${offer.name}" does not apply to plan "${plan.name}"`)
    ...
    if (offer.minPurchaseMinor !== null && input.basePriceMinor < offer.minPurchaseMinor) throw new ValidationError(`Offer requires minimum purchase ${formatMinor(offer.minPurchaseMinor, 'INR')}`)
  } else if (input.discountType !== 'NONE') {
    throw new ValidationError('Offer must be selected when discount is applied')
  }

  const pricing = calculateSalePricing({ basePriceMinor: input.basePriceMinor, discountType: input.discountType as any, discountValueMinor: input.discountValueMinor })
  if (pricing.finalPriceMinor === 0) throw new ValidationError('Final price cannot be zero — use the Trial form')

  const result = withTransaction(() => {
    const db = getDrizzle()
    const insideExisting = idempotencyRepo.find(organizationId, input.transactionId)
    if (insideExisting?.response) {
      try { return JSON.parse(insideExisting.response) as SellMembershipResult } catch {}
    }
    ...
    const durationDays = Math.floor((new Date(`${input.endDate}T00:00:00`).getTime() - new Date(`${input.startDate}T00:00:00`).getTime()) / 86400000) + 1

    const membershipRow = db.insert(memberships).values({
    ...
    const membershipId = membershipRow.id as number

    const invoiceDraft = db.insert(invoices).values({
      organization_id: organizationId,
      customer_id: customer.id,
    ...
    const invoiceId = invoiceDraft.id as number
    ...
    if (input.paidAmountMinor > lineTotalMinor) {
      throw new OverpaymentNotAllowedError(`Amount paid ${formatMinor(input.paidAmountMinor, 'INR')} exceeds invoice total ${formatMinor(lineTotalMinor, 'INR')}`)
    }
    ...
    const seqKey = ddmmYY
    let seqRow = db.select().from(invoiceSequence).where(and(eq(invoiceSequence.organization_id, organizationId), eq(invoiceSequence.year, seqKey), eq(invoiceSequence.prefix, prefix))).get() as any
    let nextVal = 1
    if (!seqRow) {
      db.insert(invoiceSequence).values({ organization_id: organizationId, year: seqKey, prefix, last_value: 1 }).run()
      nextVal = 1
    } else {
      nextVal = (seqRow.last_value as number) + 1
      db.update(invoiceSequence).set({ last_value: nextVal }).where(and(eq(invoiceSequence.organization_id, organizationId), eq(invoiceSequence.year, seqKey), eq(invoiceSequence.prefix, prefix))).run()
    }
    const invoiceNumber = `${prefix}-${ddmmYY}-${String(nextVal).padStart(2, '0')}`
    db.update(invoices).set({
      number: invoiceNumber,
      status: input.paidAmountMinor === lineTotalMinor ? 'PAID' : input.paidAmountMinor === 0 ? 'OPEN' : 'PARTIALLY_PAID',
      subtotal_minor: pricing.finalPriceMinor,
    ...
    if (input.paidAmountMinor > 0) {
      const paymentRow = db.insert(payments).values({
        organization_id: organizationId,
        customer_id: customer.id,
      ...
      paymentId = paymentRow.id as number
      const allocAmount = Math.min(input.paidAmountMinor, lineTotalMinor)
      db.insert(paymentAllocations).values({ organization_id: organizationId, payment_id: paymentId, invoice_id: invoiceId, amount_minor: allocAmount, created_by: userId }).run()
    }

    const wonStage = db.select().from(leadStages).where(and(eq(leadStages.organization_id, organizationId), eq(leadStages.is_won, true))).get() as any
    ...
    if (wonStage && !currentStage.isWon) {
      const noteType = db.select().from(leadActivityTypes).where(and(eq(leadActivityTypes.organization_id, organizationId), eq(leadActivityTypes.name, 'NOTE'))).get() as any
      if (!noteType) throw new NotFoundError('NOTE activity type is not configured')
      const conversionActivity = db.insert(leadActivities).values({
        organization_id: organizationId,
        lead_id: lead.id,
        type_id: noteType.id,
        ...
        created_by: userId
      }).returning({ id: leadActivities.id }).get()

      db.update(leads).set({ customer_id: customer.id, current_stage_id: wonStage.id, updated_at: new Date().toISOString() }).where(and(eq(leads.organization_id, organizationId), eq(leads.id, lead.id))).run()
      db.insert(leadStageHistory).values({ organization_id: organizationId, lead_id: lead.id, from_stage_id: lead.currentStageId, to_stage_id: wonStage.id, activity_id: conversionActivity.id, reason: null, changed_by: userId }).run()
    } else {
      db.update(leads).set({ customer_id: customer.id }).where(and(eq(leads.organization_id, organizationId), eq(leads.id, lead.id))).run()
    }
    ...
    if (offer) {
      db.insert(offerRedemptions).values({ organization_id: organizationId, offer_id: offer.id, membership_id: membershipId, invoice_id: invoiceId, applied_discount_minor: pricing.discountMinor, created_by: userId }).run()
    }

    const res: SellMembershipResult = { membershipId, customerId: customer.id, invoiceId, invoiceNumber, paymentId }
    db.insert(idempotencyKeys).values({ organization_id: organizationId, key: input.transactionId, response: JSON.stringify(res) }).run()
    return res
  })
