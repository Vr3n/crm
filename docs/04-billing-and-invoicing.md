...
`0.1 + 0.2` famously doesn't equal `0.3` in floating point. Do that a few hundred times across invoice lines and tax rounding, and your reported revenue will not match the sum of your invoices — a serious, embarrassing bug for a finance feature. Store money as **integer minor units** with a per-currency ISO 4217 exponent (e.g. ₹19,200.00 stored as `1920000` minor units with exponent 2; ¥1,500 stored as `1500` with exponent 0). Only format to a decimal string at the UI layer via `formatMinor`. This single decision prevents an entire category of "why doesn't this add up" bugs later.
